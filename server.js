...
const token = 'AAAAAAAAAAAAAAAAAAAAANRRmwEAAAAAE34rGEvd1tpDuSxhFHOGtujnnvE%3DOMI5BNuisBp5p1nU6M2DgZhVy19rqKSQSfIvrDnFWEt8KX7yTc'
...
// users URL
// const userURL = `https://api.twitter.com/2/users?ids=${}&user.fields=profile_image_url`
...
function streamConnect(retryAttempt) {
    ...
    stream.on('data', async(data) => {
        ...
        try {
            ...
            if(!lookup.has(json.includes.users[0].id)){
                lookup.set(json.includes.users[0].id, 0)
            }
            // console.log('Before Setting 1:', lookup)
            if(lookup.get(json.includes.users[0].id)  == 0){
            // console.log(`Tweet author ID:${json.includes.users[0].id}`)
            const profile_img = needle.get(`https://api.twitter.com/2/users?ids=${json.includes.users[0].id}&user.fields=profile_image_url`,
            {
                headers: {
                    "User-Agent": "v2FilterStreamJS",
                    "Authorization": `Bearer ${token}`
            ...
            profile_img.on('data',(profile_img)=>{
                image_url = profile_img.data[0].profile_image_url
                // console.log(`Profile_Img Id:${profile_img.data[0].id}`)
                // console.log(profile_img.data[0])
            
            if(profile_img.data[0].id === json.includes.users[0].id){
                let username = json.includes.users[0].username
                let text = json.includes.tweets[0].text
                let payload = { name : username, text: text, profile:image_url}
                io.emit("tweets",payload);
            }
            })
            lookup.set(json.includes.users[0].id, lookup.get(json.includes.users[0].id)+1)
            // console.log('After Setting 1:',lookup)
